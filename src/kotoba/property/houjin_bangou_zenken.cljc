...
(def close-cause-labels
  "登記記録の閉鎖等の事由（`closeCause`）。
   ...
   裏付けは**承継先を持つ行の事由の内訳**（実測 2026-08-19、
   `report_closure_vs_kanpou.cljs`）:

     承継先を持つ 60,753 件 = `11` 60,703 + `31` 50
     `11` の閉鎖 60,710 件のうち **7 件は承継先を持たない**

   「合併 = 承継先あり」はほぼ成り立つが**双方向とも例外がある** —— `31`
   （設立登記法人以外）にも承継が 50 件在り、合併なのに承継先が空の行が 7 件ある。
   **「11 だけが承継先を持つ」とは書かない。** 対応付けが崩れたら報告器が気付く。
