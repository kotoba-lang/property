(ns project-gleif-rr-corpus
  "GLEIF RR corpus (cache, 483k relationships) -> committed projection for the
   query plane.
   ...
   Usage:
     ...
     nbb -cp src scripts/project_gleif_rr_corpus.cljs --corpus <c> \\
       --type is-ultimately-consolidated-by --active-only \\
       --out data/gleif-relationship-ultimate.datoms.edn

   `--lei-file` matches *either* end of the edge: an edge is join-relevant when
...
(defn -main []
  (let [args (vec *command-line-args*)
    ...
    (when-not (and corpus out)
      (println "usage: project_gleif_rr_corpus.cljs --corpus <corpus.edn> --out <projection.edn> [--lei-file f] [--type t1,t2] [--validation FULLY_CORROBORATED] [--active-only]")
      (.exit js/process 2))
    (when-not (.existsSync fs corpus)
      (println (str "no corpus at " corpus " — run scripts/collect_gleif_rr_golden_copy.cljs first"))
      (.exit js/process 2))
    (let [match? (rr/matcher spec)
          tmp (str out ".partial")
             ...
             (.end sink
                   (fn []
                     (let [{:keys [manifest scanned kept unreadable]} @state
                           header (rr/projection-manifest (or manifest {}) spec kept)
                           body (.readFileSync fs tmp "utf8")]
                       (.writeFileSync fs out (str (pr-str header) "\n" body) "utf8")
                       (.unlinkSync fs tmp)
                       (when (pos? unreadable)
                         (js/console.error
                          (str "project-gleif-rr-corpus: WARNING " unreadable
